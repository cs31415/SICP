...
(define (install-scheme-number-package-ext subtype)
  ...
  (put 'div (list subtype subtype)
       (lambda (x y) (div x y)))
  (put 'square (list subtype) 
       (lambda (x) (square x)))
  (put 'atan (list subtype subtype) 
       (lambda (x y) (atan x y)))
  (put 'cos (list subtype) 
       (lambda (x) (cos x)))
  (put 'sin (list subtype) 
       (lambda (x) (sin x)))
  'done)
...
(define (install-rectangular-package)
  ;; internal procedures
  (define (real-part z) (contents-recursive (car z)))
  (define (imag-part z) (contents-recursive (cdr z)))
  ; x & y can be typed (scheme-number, integer, real, 
  ; rational etc.)
...
(define (install-polar-package)
  ;; internal procedures
  (define (magnitude z) (contents-recursive (car z)))
  (define (angle z) (contents-recursive (cdr z)))
...
(define c2 (make-complex-from-real-imag r1 rat))
c2
;(complex rectangular (real scheme-number . 3.4) rational 3 . 1)
(=zero? c2)
;#f
(=zero? (make-complex-from-real-imag 0 0))
;#t
(add c1 c2)
;(complex rectangular 5.4 . 6)
